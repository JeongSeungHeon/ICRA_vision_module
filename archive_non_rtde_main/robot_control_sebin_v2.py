...
# control loop
CONTROL_HZ =30.0
MAX_XY_SPEED_MM_S = 200
MAX_STEP_MM = MAX_XY_SPEED_MM_S / CONTROL_HZ
MAX_Z_SPEED_MM_S = 200
...
def get_instance_box_xyxy(instance):
    """
    SegmentationInstance에서 bbox를 안전하게 꺼낸다.
    지원 후보:
      - instance.box_xyxy
      - instance.bbox_xyxy
      - instance.box
    ...
    raise AttributeError(
        "Could not find bbox in SegmentationInstance. "
        "Tried box_xyxy / bbox_xyxy / box / bbox / xyxy / mask fallback."
    )

def compute_object_3d_point(instance, depth_image_m, fx, fy, cx, cy, point_mode, min_depth_m, max_depth_m):
    """
    테스트용 안정화 버전:
    mask centroid/point cloud center 대신
    bbox center + 주변 patch의 depth median으로 대표 3D point를 만든다.
    """
...
def get_close_range_step_mm(ref_err_xyz):
    """
    grasp 직전 가까운 구간에서만 step을 줄인다.
    멀 때는 기존 MAX_STEP_MM / MAX_STEP_Z_MM 그대로 사용.
    """
    dist_xy = float(np.linalg.norm(ref_err_xyz[:2]))

    # 여기 숫자는 첫 실험용 dist_xy < 45 mm일 때만 step이 줄어들어.
    close_start_mm = 45.0

    if dist_xy < close_start_mm:
        max_step_xy = 2.5
        max_step_z = 1.2
    else:
        max_step_xy = MAX_STEP_MM
        max_step_z = MAX_STEP_Z_MM

    return max_step_xy, max_step_z, dist_xy

def compute_dynamic_eef_target(object_xyz_mm, eef_xyz_mm):
    """
    멀 때는 큰 x offset으로 따라가고,
    가까워질수록 x offset을 0으로 줄인다.
    """
    obj_x, obj_y, obj_z = object_xyz_mm[:3]
    eef_x, eef_y, eef_z = eef_xyz_mm[:3]

    dist_xy = float(np.linalg.norm([obj_x - eef_x, obj_y - eef_y]))

    # 기본 접근 offset
    far_offset_x = EEF_X_OFFSET_MM   # 예: -80
    near_offset_x = 0.0
    ...
    if dist_xy >= far_d:
        ...
        t = (dist_xy - near_d) / (far_d - near_d)   # 1 -> far, 0 -> near
        offset_x = t * far_offset_x + (1.0 - t) * near_offset_x

    target_x = obj_x + offset_x
    target_y = obj_y + EEF_Y_OFFSET_MM
    target_z = obj_z

    return np.array([target_x, target_y, target_z], dtype=np.float32), float(offset_x), dist_xy
...
class FollowSharedState:
    ...
    def update_target(self, robot_xyz_m, pixel_xy, eef_xyz_mm=None):
        ...
        if eef_xyz_mm is None:
            ...
            dynamic_offset_x = EEF_X_OFFSET_MM
            dist_xy = -1.0
        else:
            target_xyz_mm, dynamic_offset_x, dist_xy = compute_dynamic_eef_target(object_xyz_mm, eef_xyz_mm)
    ...
    def should_start_pregrasp(
        ...
        """
        이제 이 함수는 '닫아도 되는 마지막 순간인가?'를 판단한다.
        object와 EEF 상대오차가 충분히 작을 때만 True.
        """
        with self.lock:
            object_xyz = None if self.latest_object_xyz_mm is None else self.latest_object_xyz_mm.copy()
...
def robot_control_loop(arm, shared_state, args):
    ...
    while not shared_state.stop_event.is_set():
        ...
        max_step_xy, max_step_z, dist_xy = get_close_range_step_mm(ref_err_xyz)
...
def execute_gripper_close(
    ...
    - gOBJ == 2 -> 성공
    - gOBJ == 1 -> 성공
    - timeout이 지나도 close command만 성공했으면 성공으로 간주
    """
    if verbose:
    ...
    while time.time() - t0 < timeout_s:
        ...
        if code == 0:
            ...
            if status is not None:
                ...
                if status["gOBJ"] in [1, 2]:
                    print("[INFO] Grasp detected by gripper status.")
                    return True

        time.sleep(poll_dt)

    print(f"[WARN] No strict grasp detected. last_status={last_status}")
    print("[INFO] But close command succeeded -> treat as grasp success for handover.")
    return True
            ...
            if selected_instances:
                ...
                if point_info is not None:
                    ...
                    if robot_xyz is not None:
                        ...
                        #     print("[INFO] INTERCEPT_READY -> PREGRASP transition")#print("[INFO] STOPPED + CLOSE_ENOUGH -> PREGRASP transition")
                        ...
                        ok_pose, cur_pose = arm.get_position() if arm is not None else (1, None)
                        eef_xyz_mm = np.array(cur_pose[:3], dtype=np.float32) if ok_pose == 0 else None
                        shared_state.update_target(robot_xyz, point_info["pixel"], eef_xyz_mm=eef_xyz_mm)
