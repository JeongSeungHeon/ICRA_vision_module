...
class GraspPointZStabilizerTests(unittest.TestCase):
    def test_uses_ref_error_xy_distance_for_activation(self):
        stabilizer = GraspPointZStabilizer(make_config())

        result = stabilizer.process(
            measured_object_point_base=(1.0, 0.5, 0.0),
    ...
    def test_large_z_jump_is_step_limited_then_ema_smoothed(self):
        stabilizer = GraspPointZStabilizer(make_config())
        stabilizer.process((0.0, 0.0, 0.0), (0.01, 0.01, 0.100))

        result = stabilizer.process((0.0, 0.0, 0.0), (0.02, 0.01, 0.200))
    ...
    def test_raw_xy_passes_through_unchanged(self):
        ...
        result = stabilizer.process((0.0, 0.0, 0.0), (0.031, 0.032, 0.200))
    ...
    def test_inactive_distance_returns_raw_z_and_resets_reentry(self):
        ...
        inactive = stabilizer.process((0.0, 0.0, 0.0), (0.07, 0.0, 0.200))
        inactive_reason = stabilizer.last_debug.reset_reason
        reentered = stabilizer.process((0.0, 0.0, 0.0), (0.01, 0.0, 0.300))
    ...
    def test_missing_inputs_clear_history_and_return_raw_or_none(self):
        ...
        raw_after_missing_object = stabilizer.process(None, (0.01, 0.01, 0.200))
        none_after_missing_grasp = stabilizer.process((0.0, 0.0, 0.0), None)
        reinitialized = stabilizer.process((0.0, 0.0, 0.0), (0.01, 0.01, 0.300))

        self.assertAlmostEqual(raw_after_missing_object[2], 0.200, places=6)
